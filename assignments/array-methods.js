...
// first and last names and populate a new array called `fullNames`. This array will contain just strings.
let fullNames = [];
...
let firstNamesAllCaps = runners.map(runner => runner.first_name.toUpperCase() + ' ' + runner.last_name);
console.log(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners 
// array, containing only those runners with large sized shirts so they can choose a different size. This will be an array
// of objects.
...
// Add up all the donations and save the total into a 
...
// Problem 1
// Select all runners with size L and donated more than 150
let list1 = runners.filter(runner => runner.shirt_size === "L" && runner.donation >= 150);
console.log(list1);

// Problem 2
// Find average donation amount
let averageDonation =  runners.reduce((acc, curr) => acc += curr.donation/runners.length, 0);
console.log(averageDonation);


// Problem 3
// Make new array for XL shirt size
let xlShirt = [];
runners.map((runner) => {
